...
func (gc *GarbageCollector) Start(ctx context.Context) error {
	...
	interval, _, _ := gc.readGCConfig(ctx)
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil
		case <-ticker.C:
			newInterval, gracePeriod, enabled := gc.readGCConfig(ctx)
			...
			if !enabled {
				if initialized {
					logger.Info("GC disabled, tearing down provider")
					initialized = false
					cloudProvider = ""
					clusterUID = ""
					p = nil
...
func (gc *GarbageCollector) readGCConfig(ctx context.Context) (interval, gracePeriod time.Duration, enabled bool) {
	...
	if v, ok := cm.Data["GC_INTERVAL"]; ok && v != "" {
		if parsed, err := time.ParseDuration(v); err == nil {
			if parsed > 0 {
				interval = parsed
			} else {
				logger.Info("WARNING: GC_INTERVAL must be positive, falling back to default", "value", v, "default", defaultGCInterval)
			}
		} else {
			logger.Error(err, "invalid GC_INTERVAL, using default", "value", v)
		}
	}

	if v, ok := cm.Data["GC_GRACE_PERIOD"]; ok && v != "" {
		if parsed, err := time.ParseDuration(v); err == nil {
			if parsed > 0 {
				gracePeriod = parsed
			} else {
				logger.Info("WARNING: GC_GRACE_PERIOD must be positive, falling back to default", "value", v, "default", defaultGCGracePeriod)
			}
		} else {
			logger.Error(err, "invalid GC_GRACE_PERIOD, using default", "value", v)
		}
	}
