...
func (p *azureProvider) CreateInstance(ctx context.Context, podName, sandboxID string, cloudConfig cloudinit.CloudConfigGenerator, spec provider.InstanceTypeSpec) (*provider.Instance, error) {
	...
	if err == nil {
		// SSH key path provided and valid, read the key
		...
		// SSH key path provided but key not found, fail
		err = fmt.Errorf("failed to find the SSH public key at %s: %w", sshPublicKeyPath, err)
		logger.Printf("%v", err)
		return nil, err
	}
