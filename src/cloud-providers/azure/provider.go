...
func (p *azureProvider) CreateInstance(ctx context.Context, podName, sandboxID string, cloudConfig cloudinit.CloudConfigGenerator, spec provider.InstanceTypeSpec) (*provider.Instance, error) {
	...
	if err != nil {
		...
		if os.IsNotExist(err) {
			errorMsg = fmt.Sprintf("ssh public key file not found: %v", err)
		} else {
			errorMsg = fmt.Sprintf("reading ssh public key file: %v", err)
		}

		logger.Printf("%s [Error Type: %v] [File: %s]\n", errorMsg, err, sshPublicKeyPath)
		return nil, err
	}
