...
func TestGetFixedIPs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			cfg: Config{
				...
				TenantName:            "test-tenant",
				DomainName:            "test-domain",
				VMStatusMaxRetries:    5,
				VMStatusRetryInterval: 5 * time.Second,
			},
		...
		},
		{
			...
			cfg: Config{
				...
				DomainName:            "test-domain",
				VMStatusMaxRetries:    5,
				VMStatusRetryInterval: 0 * time.Second,
			},
			handler:   HandlerFuncExceedsMaxRetries,
			wantError: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ips, err := GetFixedIPs(context.Background(), computeClient, &cfg, "test-server-id")
...
func TestGetFixedIPsRetryCount(t *testing.T) {
	tests := []struct {
		name            string
		cfg             Config
	...
	}{
		{
			...
			cfg: Config{
				...
				TenantName:            "test-tenant",
				DomainName:            "test-domain",
				VMStatusMaxRetries:    1,
				VMStatusRetryInterval: 5 * time.Second,
			},
			expectedRetries: 1,
		},
		{
			name: "MultipleRetries",
			cfg: Config{
				Username:              "test-user",
				Password:              "test-password",
				TenantName:            "test-tenant",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mux.HandleFunc("/v2.0/test-tenant/servers/test-server-id", HandlerFuncExceedsMaxRetries)
			...
			_, err = GetFixedIPs(context.Background(), computeClient, &cfg, "test-server-id")
			if err == nil {
				t.Fatalf("Expected error for test case %v, but got none", tt.name)
			}
			if requestCount != tt.expectedRetries {
				t.Errorf("Expected %v retries for test case %v, but got: %v", tt.expectedRetries, tt.name, requestCount)
			} else {
				t.Logf("Expected retries: %v, actual retries: %v for test case %v", tt.expectedRetries, requestCount, tt.name)
			}
		})
	}
}

func TestGetFixedIPsRetryInterval(t *testing.T) {
	tests := []struct {
		name            string
		cfg             Config
	...
	}{
		{
			...
			cfg: Config{
				...
				DomainName:            "test-domain",
				VMStatusMaxRetries:    5,
				VMStatusRetryInterval: 10 * time.Second,
			},
			minExpectedTime: 5 * 10 * time.Second,
		},
		{
			name: "ShortInterval",
			cfg: Config{
				...
				DomainName:            "test-domain",
				VMStatusMaxRetries:    5,
				VMStatusRetryInterval: 10 * time.Millisecond,
			},
			minExpectedTime: 5 * 10 * time.Millisecond,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if endTime < tt.minExpectedTime {
				t.Errorf("Expected retry duration of at least %v for test case %v, but got: %v", tt.minExpectedTime, tt.name, endTime)
			} else {
				t.Logf("Expected retry duration: %v, actual retry duration: %v for test case %v", tt.minExpectedTime, endTime, tt.name)
			}
		})
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantError {
				...
				if err != nil {
					...
					t.Logf("Successfully deleted floating IP for test case %v (no error as expected)", tt.name)
				}
			}
		})
	}
