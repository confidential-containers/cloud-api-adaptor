...
// Note: Total wait (MaxRetries * RetryInterval) must be less than proxyTimeout (default: 5 min)
// to avoid proxy timeout before server activates.
func GetFixedIPs(ctx context.Context, computeClient *gophercloud.ServiceClient, serviceConfig *Config, serverID string) ([]netip.Addr, error) {
	var ips []netip.Addr
	if serviceConfig.VMStatusMaxRetries <= 0 {
		return nil, fmt.Errorf("VMStatusMaxRetries must be a positive integer, got %d", serviceConfig.VMStatusMaxRetries)
	}
	if serviceConfig.VMStatusRetryInterval <= 0 {
		return nil, fmt.Errorf("VMStatusRetryInterval must be a positive duration, got %v", serviceConfig.VMStatusRetryInterval)
	}
	for retries := 0; retries < serviceConfig.VMStatusMaxRetries; retries++ {
		time.Sleep(serviceConfig.VMStatusRetryInterval)
	...
	return nil, fmt.Errorf("failed to get fixed IPs after %d retries", serviceConfig.VMStatusMaxRetries)
}
...
func DeleteFloatingIP(ctx context.Context, networkClient *gophercloud.ServiceClient, floatingIPID string) error {
	return floatingips.Delete(ctx, networkClient, floatingIPID).ExtractErr()
}
