	...
	tests := []struct {
	...
	}{
		{
			...
			args: []string{
				...
				"-openstack-vm-status-max-retries=30",
				"-openstack-vm-status-retry-interval=10s",
			},
			expected: Config{
		...
		},
		{
			...
			expected: Config{
				...
				DomainName:            "",
				IdentityEndpoint:      "",
				VMStatusMaxRetries:    60,
				VMStatusRetryInterval: 3 * time.Second,
			},
		},
		{
...
func comparestructs(result, expected Config) bool {
	...
	if result.VMStatusMaxRetries != expected.VMStatusMaxRetries {
		fmt.Printf("Expected VMStatusMaxRetries: %v, but got: %v\n", expected.VMStatusMaxRetries, result.VMStatusMaxRetries)
		isEqual = false
	}
	if result.VMStatusRetryInterval != expected.VMStatusRetryInterval {
		fmt.Printf("Expected VMStatusRetryInterval: %v, but got: %v\n", expected.VMStatusRetryInterval, result.VMStatusRetryInterval)
		isEqual = false
	}
