...
func CreateDomain(ctx context.Context, libvirtClient *libvirtClient, v *vmConfig) (result *createDomainOutput, err error) {
	...
	if err == nil {
		...
		if err != nil {
			return nil, fmt.Errorf("existing domain UUID retrieval failed: %w", err)
		}
		v.instanceID = uuid
		if v.ips, err = getDomainIPs(domain); err != nil {
			// Domain exists and UUID is valid; return the instance so the caller
			// can clean up even if IP retrieval fails (e.g. domain is shut off).
			logger.Printf("existing domain IP retrieval failed (uuid=%s): %v", uuid, err)
			return &createDomainOutput{instance: v}, fmt.Errorf("existing domain IP retrieval failed: %w", err)
		}
