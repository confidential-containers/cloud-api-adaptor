...
if ! ls /etc/ssh/sshd_config.bak.* &>/dev/null; then
    cp "${SSH_CONFIG}" "/etc/ssh/sshd_config.bak.$(date +%F_%T)"
fi

# Append SSH config to disable login (guarded so re-runs don't duplicate it)
if [[ "$DISABLE_SSH_LOGIN" == "true" ]] && ! grep -qE "^\s*#\s*Disable all forms of login" "$SSH_CONFIG"; then
cat >> "$SSH_CONFIG" <<EOF
