...
gcp() {
    ...
    [[ "${PODVM_IMAGE_NAME}" ]] && optionals+="-imageid ${PODVM_IMAGE_NAME} "
    ...
    [[ "${GCP_MACHINE_TYPE}" ]] && optionals+="-instance-type ${GCP_MACHINE_TYPE} "    # default e2-medium
    [[ "${GCP_NETWORK}" ]] && optionals+="-subnetid ${GCP_NETWORK} "                   # defaults to 'default'
