...
func DoTestHTTPSKbsKeyRelease(t *testing.T, e env.Environment, assert CloudAssert, kbsEndpoint, workerNodeIP, resourcePath, expectedSecret string) {
	t.Log("Do test https kbs key release")
	fmt.Printf("workernode ip: %s", workerNodeIP)
	pod := NewBusyboxPodWithNameWithInitdatahttps(E2eNamespace, "kbs-key-release", kbsEndpoint, workerNodeIP).GetPodOrFatal(t)
	testCommands := []TestCommand{
		{
			Command:       []string{"wget", "-q", "-O-", "http://127.0.0.1:8006/cdh/resource/" + resourcePath},
			ContainerName: pod.Spec.Containers[0].Name,
...
func DoTestHTTPSKbsKeyReleaseForFailure(t *testing.T, e env.Environment, assert CloudAssert, kbsEndpoint, workerNodeIP, resourcePath, expectedSecret string) {
	t.Log("Do test kbs key release failure case")
	pod := NewBusyboxPodWithNameWithInitdatahttps(E2eNamespace, "kbs-failure", kbsEndpoint, workerNodeIP).GetPodOrFatal(t)
	testCommands := []TestCommand{
		{
			Command:       []string{"wget", "-q", "-O-", "http://127.0.0.1:8006/cdh/resource/" + resourcePath},
			ContainerName: pod.Spec.Containers[0].Name,
