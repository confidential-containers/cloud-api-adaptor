...
func DoTestCreatePodWithSecret(t *testing.T, e env.Environment, assert CloudAssert) {
	...
	pod := NewPod(E2eNamespace, podName, containerName, imageName, WithSecretBinding(podKubeSecretsDir, secretName, containerName), WithCommand([]string{"/bin/sh", "-c", "sleep 3600"}))
...
func DoTestPodsMTLSCommunication(t *testing.T, e env.Environment, assert CloudAssert) {
	...
	clientPod := NewExtraPod(
		...
		WithSecretBinding(clientSecretDir, clientSecretName, clientContainerName),
	...
	serverPod := NewPod(E2eNamespace, serverPodName, serverContainerName, serverImageName, WithSecureContainerPort(443), WithSecretBinding(serverSecretDir, serverSecretName, serverContainerName), WithLabel(labels), WithConfigMapBinding(podKubeConfigmapDir, configMapName))
	configMap := NewConfigMap(E2eNamespace, configMapName, configMapData)
