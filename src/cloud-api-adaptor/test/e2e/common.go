...
func GetContainerIndex(pod *corev1.Pod, containerName string) *corev1.Container {
	for i := range pod.Spec.Containers {
		if pod.Spec.Containers[i].Name == containerName {
			return &pod.Spec.Containers[i]
		}
	}
	log.Fatalf("container %q not found in pod spec", containerName)
	return nil
}
...
func WithSecretBinding(mountPath string, secretName string, containerName string) PodOption {
	return func(p *corev1.Pod) {
		if c := GetContainerIndex(p, containerName); c != nil {
...
func WithPVCBinding(mountPath string, pvcName string, containerName string) PodOption {
