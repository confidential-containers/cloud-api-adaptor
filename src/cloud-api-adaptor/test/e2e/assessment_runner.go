		...
		Teardown(func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			if tc.job != nil {
				...
				if t.Failed() {
					if len(podList.Items) > 0 {
						...
						podLogString, err := GetPodLog(ctx, client, &jobPod)
						if err != nil {
							t.Error(err)
						}
						t.Logf("Job pod log: %s\n", podLogString)

						caaPodLog, err := GetCaaPodLog(ctx, t, client, &jobPod)
						if err != nil {
							t.Error(err)
						}
						t.Logf("CAA log: %s\n", caaPodLog)
			...
			if tc.pod != nil {
				...
				if t.Failed() {
					...
					caaPodLog, err := GetCaaPodLog(ctx, t, client, tc.pod)
					if err != nil {
						t.Error(err)
					}
					t.Logf("CAA log: %s\n", caaPodLog)
