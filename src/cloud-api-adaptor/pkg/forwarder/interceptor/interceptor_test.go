...
func TestIsLuks(t *testing.T) {
	t.Run("detects valid LUKS header", func(t *testing.T) {
		f := filepath.Join(t.TempDir(), "luks-device")
		// LUKS magic: "LUKS" + 0xBA 0xBE followed by some padding
		data := append([]byte{'L', 'U', 'K', 'S', 0xba, 0xbe}, make([]byte, 100)...)
	...
	t.Run("returns false for non-LUKS device", func(t *testing.T) {
		f := filepath.Join(t.TempDir(), "plain-device")
		data := make([]byte, 512)
		require.NoError(t, os.WriteFile(f, data, 0o644))
	...
	t.Run("returns error for partial header", func(t *testing.T) {
		f := filepath.Join(t.TempDir(), "short-device")
		data := []byte{'L', 'U', 'K'}
		require.NoError(t, os.WriteFile(f, data, 0o644))
	...
	t.Run("returns error for nonexistent device", func(t *testing.T) {
		result, err := isLuks("/dev/nonexistent-device-xyz")
		assert.Error(t, err)
		assert.False(t, result)
	})

	t.Run("detects LUKS2 header (same magic)", func(t *testing.T) {
		f := filepath.Join(t.TempDir(), "luks2-device")
		// LUKS magic is the same for v1 and v2; version is in bytes 6-7
		data := append([]byte{'L', 'U', 'K', 'S', 0xba, 0xbe, 0x00, 0x02}, make([]byte, 100)...)
		require.NoError(t, os.WriteFile(f, data, 0o644))
