...
func main() {
	...
	if err := report(ctx, client, *endpoint, *verbose); err != nil {
		log.Fatalf("reporting ready failed: %v", err)
	}
...
func negotiateVersion(ctx context.Context, c *http.Client, endpoint string, verbose bool) (string, error) {
	...
	if len(v.Supported.Version) > 0 {
		supportedVersion := v.Supported.Version[0]
		log.Printf("preferred version %q not found, falling back to %q", defaultVersion, supportedVersion)
