...
| Alibaba Cloud | Supported | @Xynnn007 | Daily e2e CI (schedule/dispatch only, not on PRs). Works with Intel TDX instances. |
| Azure | Supported | @mkulke | Daily e2e CI (schedule/dispatch only, not on PRs). Supports Intel TDX and AMD SEV-SNP CVMs with vTPM. |
| BYOM (Bring Your Own Machine) | Supported | @bpradipt, @Amulyam24 | Daily e2e CI. Uses pre-created VMs via SSH/SFTP.|
| AWS | Best effort | @EmmEff, @snir911 | Limited development/maintenance. Daily e2e CI. Works with non-TEE and AMD SEV-SNP instances. |
| GCP | Best effort| @EmmEff, @beraldoleal, @snir911 | No upstream CI. Limited development/maintenance. Documentation exists. Works with SEV-SNP and TDX instances |
