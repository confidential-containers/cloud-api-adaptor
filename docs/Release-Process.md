...
Helm chart. The release workflow tags images with the release version (e.g. `v0.18.0`), so this step must happen
after those images are available.

Update `image.tag` in [`values.yaml`](../src/cloud-api-adaptor/install/charts/peerpods/values.yaml) from `"latest"`
to the release version (e.g. `v0.18.0`). Then trigger the
...
Revert `image.tag` in [`values.yaml`](../src/cloud-api-adaptor/install/charts/peerpods/values.yaml) back to `"latest"`.
